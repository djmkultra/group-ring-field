...
#ifndef __GUTZ_DEVICE_EVENT_DOT_H
#define __GUTZ_DEVICE_EVENT_DOT_H
...
public:
   ...
   DeviceEvent( const DeviceEvent &me )
     : CameraEvent( me ), _button(me._button), _down(me._down), 
     _pos(me._pos), pickData(me.pickData)
       {
	 setWorldPos();
       }
...
typedef gutz::SmartPtr<DeviceEvent> DeviceEventSP;

///////////////////////////////////////////////////////////////////////////
/// DeviceMoveEvent, the mouse is moving.  
///   Be sure to look up the hierarchy for more interface stuff... 
///////////////////////////////////////////////////////////////////////////

class DeviceMoveEvent : public DeviceEvent {
public:
   DeviceMoveEvent(const vec3f &pos, const vec3f &last,
                  bool down = true, unsigned int button = gutz::GUTZ_BUTTON_NONE,
                  const CameraSP cam=0, const ManipSP manip=0)
      : DeviceEvent(pos, down, button, cam, manip), _last(last)
   {setWorldLast();}
   /// for initallizing from a preceeding DeviceEvent
   DeviceMoveEvent(const DeviceEvent &me, const vec3f &curPos)
      : DeviceEvent(curPos, me.isButtonDown(), me.getButton(), me.pickData, 
                   me.getCamera(), me.getManip()),
        _last(me.getPos())
   {setWorldLast();}
  DeviceMoveEvent( const DeviceMoveEvent &mme )
    : DeviceEvent( mme ), _last(mme._last), _wlast(mme._wlast)
    { setWorldLast();}

   /// using default copy and assignment
   virtual ~DeviceMoveEvent() {}

   /// update current position, copies old pos to last pos, then sets current.
   void setPos(const vec3f &pos)       
   { _last = getPos(); _wlast = getWorldPos(); DeviceMoveEvent::setPos(pos); }
...
protected:
   ...
   DeviceMoveEvent(); /// not used
   vec3f _last;
   vec3f _wlast;

};

typedef gutz::SmartPtr<DeviceMoveEvent> DeviceMoveEventSP;
